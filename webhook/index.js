...
remark = remark().use(math).use(strip);
...
handler.on("push", (event) => {
  ...
  exec("bash update.sh", (err) => {
    ...
    removed = [...removed].filter(
      (e) => e.startsWith("docs") && e.endsWith("md"),
    );
    ...
    removed.forEach(
      (file, index) => (removed[index] = file.replace("docs/", "")),
    );
    modified.forEach(
      (file, index) => (modified[index] = file.replace("docs/", "")),
    );
